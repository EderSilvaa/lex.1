...
import { searchFiles } from '../dropbox-client';

export const dropboxBuscar: Skill = {
    nome: 'dropbox_buscar',
    descricao: 'Busca arquivos no Dropbox por nome ou conteúdo.',
    categoria: 'dropbox',
    ...
    async execute(params: Record<string, any>, _ctx: AgentContext): Promise<SkillResult> {
        ...
        try {
            ...
            const formatted = results.map((r: any) => `• ${r['name'] || r['path_display'] || '?'}`).join('\n');
