"""
Interactive setup wizard for Lex_Engine.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("Lex_Engine Setup - Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Lex_Engine using environment variables or config commands:")
...
# between `hermes tools` and `Lex_Engine setup tools`).
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'Lex_Engine setup tts')."""
    _setup_tts_provider(config)
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Hermes",
            bot_description="Your Lex. agent on Slack",
        )
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Lex. from anywhere.")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `Lex_Engine setup tools` and `hermes tools` use the same flow:
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Lex_Engine config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Lex_Engine.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" differs from Lex_Engine's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    # Execute the migration — overwrite=False so existing Lex_Engine configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Lex_Engine config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      Lex_Engine setup           — full or quick (auto-detected)
      Lex_Engine setup model     — just model/provider
      Lex_Engine setup tts       — just text-to-speech
      Lex_Engine setup terminal  — just terminal backend
      Lex_Engine setup gateway   — just messaging platforms
      Lex_Engine setup tools     — just tool configuration
      Lex_Engine setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│      Lex_Engine Setup — {label:<33s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│              Lex_Engine Setup Wizard                  │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Lex_Engine installation.         │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Lex_Engine configured.")
        ...
        setup_mode = prompt_choice("How would you like to set up Lex_Engine?", [
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Lex. to messaging apps to chat from anywhere.")
