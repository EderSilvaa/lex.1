"""
Multi-provider authentication system for Lex_Engine.
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Lex_Engine:")
    print(authorize_url)
    print()
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Lex_Engine's own Codex token store."""
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Lex_Engine processes
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Lex_Engine's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Lex_Engine's next refresh uses it, and the whole
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Lex_Engine is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Lex_Engine is not logged into Nous Portal.",
...
def login_command(args) -> None:
    """Deprecated: use 'hermes model' or 'Lex_Engine setup' instead."""
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Lex_Engine auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Lex_Engine will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in ("y", "yes"):
                ...
                print("Lex_Engine will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return
    ...
    print("(Lex_Engine creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Lex_Engine login via {pconfig.name}...")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or config_matches:
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Lex_Engine will use OpenRouter for inference.")
        else:
            print("Run `hermes model` or configure an API key to use Lex_Engine.")
    else:
