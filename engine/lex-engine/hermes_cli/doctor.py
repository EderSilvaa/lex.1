"""
Doctor command for Lex_Engine CLI.

Diagnoses issues with Lex_Engine setup.
"""
...
def run_doctor(args):
    ...
    print(color("│                 Lex_Engine Doctor                      │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Lex. a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Lex. Persona\n\n"
                "<!-- Edit this file to customize how Lex. communicates. -->\n\n"
                "You are Lex., a helpful legal AI assistant.\n",
                encoding="utf-8",
            )
