"""Slash command definitions and autocomplete for the Lex_Engine CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    # Sessão
    CommandDef("new", "Inicia uma nova sessão (ID e histórico zerados)", "Sessão",
               aliases=("reset",)),
    CommandDef("clear", "Limpa a tela e inicia uma nova sessão", "Sessão",
               cli_only=True),
    CommandDef("redraw", "Força redesenho da UI (recupera após desvio do terminal)", "Sessão",
               cli_only=True),
    CommandDef("history", "Mostra o histórico da conversa", "Sessão",
               cli_only=True),
    CommandDef("save", "Salva a conversa atual", "Sessão",
               cli_only=True),
    CommandDef("retry", "Reenvia a última mensagem ao agente", "Sessão"),
    CommandDef("undo", "Remove a última troca usuário/assistente", "Sessão"),
    CommandDef("title", "Define um título para a sessão atual", "Sessão",
               args_hint="[nome]"),
    CommandDef("branch", "Cria um branch da sessão (explora tese ou caminho alternativo)", "Sessão",
               aliases=("fork",), args_hint="[nome]"),
    CommandDef("compress", "Compacta o contexto da conversa manualmente", "Sessão",
               args_hint="[foco]"),
    CommandDef("rollback", "Lista ou restaura checkpoints do filesystem", "Sessão",
               args_hint="[número]"),
    CommandDef("snapshot", "Cria ou restaura snapshots de configuração/estado do Lex Engine", "Sessão",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "Encerra todos os processos rodando em background", "Sessão"),
    CommandDef("approve", "Aprova um comando perigoso pendente (HITL)", "Sessão",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "Nega um comando perigoso pendente (HITL)", "Sessão",
               gateway_only=True),
    CommandDef("background", "Executa um prompt em background", "Sessão",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "Mostra agentes ativos e tarefas em execução", "Sessão",
               aliases=("tasks",)),
    CommandDef("queue", "Enfileira um prompt para o próximo turno (sem interromper)", "Sessão",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "Injeta uma mensagem após a próxima chamada de tool (sem interromper)", "Sessão",
               args_hint="<prompt>"),
    CommandDef("status", "Mostra informações da sessão", "Sessão"),
    CommandDef("profile", "Mostra o perfil ativo e o diretório home", "Info"),
    CommandDef("sethome", "Define este chat como canal principal", "Sessão",
    ...
    CommandDef("config", "Mostra a configuração atual", "Configuração",
               cli_only=True),
    CommandDef("model", "Troca o modelo desta sessão", "Configuração",
               aliases=("provider",), args_hint="[modelo] [--provider nome] [--global]"),
    CommandDef("gquota", "Mostra uso da quota do Google Gemini Code Assist", "Info",
               cli_only=True),

    CommandDef("personality", "Define uma personalidade predefinida (tom de resposta)", "Configuração",
               args_hint="[nome]"),
    CommandDef("statusbar", "Liga/desliga a barra de status (contexto/modelo)", "Configuração",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "Cicla a exibição de progresso de tools: off → new → all → verbose",
               "Configuração", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("yolo", "Liga/desliga o modo YOLO (pula todas as aprovações de comandos perigosos)",
               "Configuração"),
    CommandDef("reasoning", "Gerencia esforço e exibição do raciocínio", "Configuração",
               args_hint="[nível|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "Liga/desliga modo rápido — Priority Processing (OpenAI) / Fast Mode (Anthropic)", "Configuração",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "Mostra ou troca o skin/tema da interface", "Configuração",
               cli_only=True, args_hint="[nome]"),
    CommandDef("voice", "Liga/desliga modo de voz", "Configuração",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status")),
    CommandDef("busy", "Controla o que o Enter faz enquanto a Lex está trabalhando", "Configuração",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Ferramentas & Skills
    CommandDef("tools", "Gerencia tools: /tools [list|disable|enable] [nome...]", "Ferramentas & Skills",
               args_hint="[list|disable|enable] [nome...]", cli_only=True),
    CommandDef("toolsets", "Lista os toolsets disponíveis", "Ferramentas & Skills",
               cli_only=True),
    CommandDef("skills", "Busca, instala, inspeciona ou gerencia skills (inclui as jurídicas)",
               "Ferramentas & Skills", cli_only=True,
               subcommands=("search", "browse", "inspect", "install")),
    CommandDef("cron", "Gerencia tarefas agendadas (ex.: monitorar publicações, prazos)", "Ferramentas & Skills",
               cli_only=True, args_hint="[subcomando]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("kanban", "Quadro de colaboração multi-perfil (tarefas, links, comentários)",
               "Ferramentas & Skills", args_hint="[subcomando]",
    ...
    CommandDef("reload", "Recarrega variáveis do .env na sessão em execução", "Ferramentas & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "Recarrega os servers MCP da configuração", "Ferramentas & Skills",
               aliases=("reload_mcp",)),
    CommandDef("browser", "Conecta as tools de browser ao seu Chrome ao vivo via CDP", "Ferramentas & Skills",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "Lista os plugins instalados e seus status",
               "Ferramentas & Skills", cli_only=True),

    # Info
    CommandDef("commands", "Navega por todos os comandos e skills (paginado)", "Info",
               gateway_only=True, args_hint="[página]"),
    CommandDef("help", "Mostra os comandos disponíveis", "Info"),
    CommandDef("restart", "Reinicia o gateway com segurança após drenar runs ativos", "Sessão",
               gateway_only=True),
    CommandDef("usage", "Mostra uso de tokens e rate limits da sessão atual", "Info"),
    CommandDef("insights", "Mostra insights e analytics de uso", "Info",
               args_hint="[dias]"),
    CommandDef("platforms", "Mostra status das plataformas de gateway/mensageria", "Info",
               cli_only=True, aliases=("gateway",)),
    CommandDef("copy", "Copia a última resposta do assistente para a área de transferência", "Info",
               cli_only=True, args_hint="[número]"),
    CommandDef("paste", "Anexa imagem da área de transferência", "Info",
               cli_only=True),
    CommandDef("image", "Anexa um arquivo de imagem local ao próximo prompt", "Info",
               cli_only=True, args_hint="<caminho>"),
    CommandDef("update", "Atualiza o Lex Engine para a última versão", "Info",
               gateway_only=True),
    CommandDef("debug", "Envia relatório de debug (info do sistema + logs) e devolve links compartilháveis", "Info"),

    # Sair
    CommandDef("quit", "Sai da CLI", "Sair",
