#!/usr/bin/env python3
"""
Lex_Engine CLI - Main entry point.
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (hermes tools, Lex_Engine setup, hermes model) use
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Lex_Engine isn't configured yet -- no API keys or providers found."
        )
        print()
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Lex.?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with 'Lex.'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Lex_Engine only falls back to a lightweight backend (OpenRouter,")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  Lex_Engine will probe your")
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Lex_Engine turns to `copilot --acp`.")
    print("  Lex_Engine currently starts its own ACP subprocess for each request.")
    print("  Lex_Engine uses your selected model as a hint for the Copilot ACP session.")
...
def cmd_login(args):
    """Authenticate Lex_Engine CLI with a provider."""
...
def cmd_version(args):
    """Show version."""
    print(f"Lex_Engine v{__version__} ({__release_date__})")
...
def _update_via_zip(args):
    """Update Lex_Engine by downloading a ZIP archive.
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Lex_Engine behaves unexpectedly.")
    ...
    print("  Review `git diff` / `git status` if Lex_Engine behaves unexpectedly.")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("Your fork is not tracking the official upstream repository.")
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Lex_Engine — ensure /usr/local/bin is on PATH "
...
def cmd_update(args):
    """Update Lex_Engine to the latest version.
    ...
    if is_managed():
        managed_error("update Lex_Engine")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Warn if legacy Lex_Engine gateway unit files are still installed.
...
def main():
    """Main entry point for Lex_Engine CLI."""
    parser = argparse.ArgumentParser(
        prog="hermes",
        description="Lex. - AI assistant with tool-calling capabilities",
...
""",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Lex.",
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Lex_Engine with an interactive wizard. "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Lex.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Lex.")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Lex_Engine CLI",
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate Lex_Engine with Spotify via PKCE")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Lex_Engine components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Lex_Engine setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Lex_Engine setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Lex_Engine. Use 'hermes debug share' to "
...
""",
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Lex_Engine home directory to a zip file",
        description="Create a zip archive of your entire Lex_Engine configuration, "
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Lex_Engine backup from a zip file",
        description="Extract a previously created Lex_Engine backup into your "
        "Lex_Engine home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Lex_Engine configuration",
    )
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Lex_Engine as an MCP server",
        description=(
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Lex_Engine as an MCP server (expose conversations to other agents)",
    )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Lex_Engine",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Lex_Engine",
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Lex_Engine to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Lex_Engine",
        description="Remove Lex_Engine from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Lex_Engine as an ACP (Agent Client Protocol) server",
        description="Start Lex_Engine in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Lex_Engine as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Lex_Engine instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Lex_Engine web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Lex_Engine log files",
