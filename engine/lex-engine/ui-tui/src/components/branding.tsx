...
export function Banner({ t }: { t: Theme }) {
  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      ...
      <Text color={t.color.dim}>Lex_Engine</Text>
    </Box>
  )
}

export function SessionPanel({ info, t }: SessionPanelProps) {
  const cols = useStdout().stdout?.columns ?? 100
  const w = Math.max(20, cols - 12)
  const lineBudget = Math.max(12, w - 2)
  const strip = (s: string) => (s.endsWith('_tools') ? s.slice(0, -6) : s)
  const displayItem = (s: string) =>
    s === 'hermes-agent' ? 'lex-engine-agent' : s === 'hermes-agent-dev' ? 'lex-engine-dev' : s

  const truncLine = (pfx: string, items: string[]) => {
    let line = ''
    let shown = 0

    for (const item of [...items].sort().map(displayItem)) {
